...
int main(int argc, char **argv){
    ...
    cv::glob("/home/elie/projects/camera_calibration/C++/camera_calibration/calibration/Image*.png", fileNames, false);
    cv::Size patternSize(25 - 1, 18 - 1);
    std::vector<std::vector<cv::Point2f>> q(fileNames.size());

    std::vector<std::vector<cv::Point3f>> Q;

    // 1. Generate checkerboard (world) coordinates Q. The board has 25x18 fields with a size of 15mmx15mm
    ...
    for(int i = 1; i<checkerBoard[1]; i++){
        for(int j = 1; j<checkerBoard[0]; j++){
            objp.push_back(cv::Point3f(j*fieldSize, i*fieldSize,0)); // no z axis so z=0
        }
    }
    ...
    // 2. Read the image and call cv::findChessboardCorners()
